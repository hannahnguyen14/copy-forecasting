...
def dynamic_topK(A: torch.Tensor, k: int) -> torch.Tensor:
    """
    A: [B, N, N] adjacency (unnormalized)
    k: số neighbors mong muốn
    Trả về một mask mà chỉ giữ lại top‐k (theo trọng số) trên mỗi hàng.
    """
    B, N, _ = A.shape
    # đảm bảo k không vượt quá N
    k_clipped = min(k, N)

    # cộng nhiễu nhỏ để tránh trường hợp bằng nhau
    A_noise = A + torch.rand_like(A) * 0.01
    # lấy top‐k_clipped phần tử theo chiều cuối
    values, indices = A_noise.topk(k_clipped, dim=2)

    # xây mask
    mask = torch.zeros_like(A)
    # với mỗi batch, mỗi node i, đánh dấu những indices[i] lên mask = 1
    batch_idx = torch.arange(B).view(B, 1, 1).expand(-1, N, k_clipped)
    node_idx = torch.arange(N).view(1, N, 1).expand(B, -1, k_clipped)
    mask[batch_idx, node_idx, indices] = 1.0

    # trả về adjacency đã lọc
    return A * mask
