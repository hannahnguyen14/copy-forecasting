...
from tsfb.base.origin_code.tgcrn.model.GCGRU import (
    GCGRU,
    GRUCell,
    Time2Vec,
    TimeEncode,
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.gcgru_cells.append(
            GCGRU(node_num, in_dim, out_dim, cheb_k, embed_dim, time_station, per_flag)
        )
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.gcgru_cells.append(
            GCGRU(
                node_num, in_dim, rnn_units, cheb_k, embed_dim, time_station, per_flag
            )
        )
        for _ in range(1, num_layers):
            self.gcgru_cells.append(
...
class TGCRN(nn.Module):
    def __init__(self, args):
        ...
        if args.graph_direction == "symm":
            self.node_embeddings = nn.Parameter(
                torch.randn(self.num_nodes, args.embed_dim), requires_grad=True
            )
        ...
        self.node_weight = nn.Parameter(
            torch.randn(args.embed_dim, args.embed_dim), requires_grad=True
        )
        self.time_weight = nn.Parameter(
            torch.randn(args.time_dim, args.time_dim), requires_grad=True
        )
    ...
    def forward(self, _input):
        ...
        if self.args.od_flag:
            ...
            output, _ = self.encoder(
                _input, init_state, self.node_embeddings
            )  # output: [B, T, N, hidden]
        ...
        if self.args.Seq_Dec:
            ...
            output = output.squeeze(-1).reshape(
                -1, self.horizon, self.output_dim, self.num_nodes
            )
        ...
        if self.args.constrative_time and self.training:
            ...
            for i in range(1, 4):
                embed_diff = torch.mean(
                    (cons_time_embed[:, 0, :] - cons_time_embed[:, i, :]) ** 2,
                    dim=-1,
                    keepdim=True,
                )  # 16 x 1
